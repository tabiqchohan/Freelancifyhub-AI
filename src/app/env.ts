 ...
 * AG-003 knowledge config, and the AG-004 tool config derived from the same
 * process environment. Secrets are never logged or surfaced; they are read once
 * here and consumed downstream. The tool database URL is reused from the shared
 * config when present (never duplicated).
 */
...
export function parseCompiledEnv(raw: NodeJS.ProcessEnv = process.env): Environment {
  ...
  return { base, memory, knowledge, tools };
}
