/**
 * Sprint 23 — Marketplace AI Team v1. Security & output redaction utilities.
 *
 * Marketplace content (project descriptions, messages, portfolio text, tool
 * results, knowledge documents) is all untrusted DATA — never instructions
 * (Sprint 23 §22/§23). The v1 marketplace agents are deterministic, so they
 * never build prompts; these helpers guarantee that any value crossing the
 * marketplace boundary is bounded, boundary markers are neutralized and
...
/** Upper bound on any single string passed to the marketplace boundary. */
export const MARKETPLACE_SECURITY_MAX_VALUE_BYTES = 65_536;

/**
 * Sanitizes a marketplace-facing text: collapses whitespace, neutralizes the
...
export function sanitizeMarketplaceText(
  value: string,
  maxBytes = MARKETPLACE_SECURITY_MAX_VALUE_BYTES,
): string {
 ...
 */
export function redactMarketplaceValue(value: unknown): unknown {
 ...
 * the primary guard. Primary defense is treating all marketplace content as
...
export function assertInputPayloadSafe(input: unknown, allowInjectionShaped = false): void {
  if (!allowInjectionShaped && containsInjectionIndicators(input)) {
    throw new MarketplaceAIError(
      MARKETPLACE_AI_ERROR_CODES.PROMPT_INJECTION,
...
export function safeMarketplaceValue(
  value: unknown,
  maxBytes = MARKETPLACE_SECURITY_MAX_VALUE_BYTES,
): unknown {
...
function sanitizeRecursively(value: unknown, maxBytes: number): unknown {
  if (typeof value === 'string') {
    return sanitizeMarketplaceText(value, maxBytes);
  }
  if (Array.isArray(value)) {
