/**
 * Sprint 25 — Admin AI Team v1. Privileged, deterministic metrics.
 *
 * Lightweight counters/gauges for the admin-AI service, including explicit
 * authorization-denial, tool-denial and approval-required counters so that
 * privileged-path behavior (BR-ADM-1..4) is observable. All numbers are
 * locally aggregated; nothing sensitive is ever stored.
 */

/** Counter snapshot for the admin-AI team. */
export interface AdminAICounters {
...
/** Gauge snapshot for the admin-AI team. */
export interface AdminAIGauges {
...
export interface AdminAIMetricsSnapshot {
  readonly counters: AdminAICounters;
  readonly gauges: AdminAIGauges;
  readonly averageLatencyMs: number;
}

/** Deterministic metrics for the admin-AI service. */
export class AdminAIMetrics {
  readonly name = 'admin-ai-metrics';
  ...
  /** Records a capability that honestly reported insufficient data. */
  recordInsufficientData(): void {
    this.insufficientData += 1;
  }

  /** Records a denied authorization (UNAUTHORIZED or FORBIDDEN). */
  recordAuthorizationDenial(): void {
    this.authorizationDenials += 1;
  }

  /** Records a denied tool call (strictly before tool I/O). */
  recordToolDenial(): void {
    this.toolDenials += 1;
  }

  /** Records a recommendation that requires explicit approval (BR-ADM-2). */
  ...
  snapshot(): AdminAIMetricsSnapshot {
