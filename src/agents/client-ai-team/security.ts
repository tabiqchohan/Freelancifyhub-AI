/**
 * Sprint 21 — Client AI Team v1. Security & output redaction utilities.
 *
 * Client text, retrieved memory, knowledge documents and tool output are all
 * treated as untrusted DATA — never as instructions (Sprint 21 §17). The v1
 * client agents are deterministic, so they never build prompts; these helpers
 * guarantee that any value crossing the client boundary is bounded, boundary
 * markers are neutralized and credential-looking content is redacted.
 */
...
/** Upper bound on any single string passed to the client boundary. */
export const CLIENT_SECURITY_MAX_VALUE_BYTES = 65_536;

/**
 * Sanitizes a client-facing text: collapses whitespace, neutralizes the
...
export function sanitizeClientText(
  value: string,
  maxBytes = CLIENT_SECURITY_MAX_VALUE_BYTES,
): string {
 ...
 * the primary guard. Primary defense is treating all client text as data in
 * the deterministic engine and never building prompts from it.
 */
...
export function assertInputPayloadSafe(input: unknown, allowInjectionShaped = false): void {
  if (!allowInjectionShaped && containsInjectionIndicators(input)) {
    throw new ClientAIError(
      CLIENT_AI_ERROR_CODES.PROMPT_INJECTION,
...
export function safeClientValue(
  value: unknown,
  maxBytes = CLIENT_SECURITY_MAX_VALUE_BYTES,
): unknown {
...
function sanitizeRecursively(value: unknown, maxBytes: number): unknown {
  if (typeof value === 'string') {
    return sanitizeClientText(value, maxBytes);
  }
  if (Array.isArray(value)) {
