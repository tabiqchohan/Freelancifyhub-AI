/**
 * Sprint 24 — Marketing AI Team v1. Security & output redaction utilities.
 *
 * Marketing content (research briefs, social posts, blog drafts, email copy,
 * page content, tool results, knowledge documents) is all untrusted DATA —
 * never instructions (Sprint 24 §6). The v1 marketing agents are deterministic,
 * so they never build prompts; these helpers guarantee that any value crossing
 * the marketing boundary is bounded, boundary markers are neutralized and
 * credential-looking content is redacted.
 */
...
/** Upper bound on any single string passed to the marketing boundary. */
export const MARKETING_SECURITY_MAX_VALUE_BYTES = 65_536;

/**
 * Sanitizes a marketing-facing text: collapses whitespace, neutralizes the
 * untrusted-context boundary token, redacts secrets, then truncates to an
 * explicit byte budget. Never throws for valid strings.
 */
export function sanitizeMarketingText(
  value: string,
  maxBytes = MARKETING_SECURITY_MAX_VALUE_BYTES,
): string {
 ...
 */
export function redactMarketingValue(value: unknown): unknown {
 ...
 * the primary guard. Primary defense is treating all marketing content as
 * data in the deterministic engine and never building prompts from it.
 */
...
export function assertInputPayloadSafe(input: unknown, allowInjectionShaped = false): void {
  if (!allowInjectionShaped && containsInjectionIndicators(input)) {
    throw new MarketingAIError(
      MARKETING_AI_ERROR_CODES.PROMPT_INJECTION,
...
export function safeMarketingValue(
  value: unknown,
  maxBytes = MARKETING_SECURITY_MAX_VALUE_BYTES,
): unknown {
...
function sanitizeRecursively(value: unknown, maxBytes: number): unknown {
  if (typeof value === 'string') {
    return sanitizeMarketingText(value, maxBytes);
  }
  if (Array.isArray(value)) {
