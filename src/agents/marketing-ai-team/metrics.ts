/**
 * Sprint 24 — Marketing AI Team v1. Deterministic metrics.
 *
 * Lightweight counters/gauges for the marketing-AI service. All numbers are
 * locally aggregated; nothing sensitive is ever stored. `insufficientData`
 * and `drafting` states (Sprint 24 §13) are reported whenever a marketing
 * capability honestly reports it cannot derive an observation or a complete
 * draft from the supplied data.
 */

/** Counter snapshot for the marketing-AI team. */
export interface MarketingAICounters {
...
/** Gauge snapshot for the marketing-AI team. */
export interface MarketingAIGauges {
...
export interface MarketingAIMetricsSnapshot {
  readonly counters: MarketingAICounters;
  readonly gauges: MarketingAIGauges;
  readonly averageLatencyMs: number;
}

/** Deterministic metrics for the marketing-AI service. */
export class MarketingAIMetrics {
  readonly name = 'marketing-ai-metrics';
  ...
  /** Records a capability that honestly reported insufficient data (§13). */
  ...
  snapshot(): MarketingAIMetricsSnapshot {
